// Admin bookmarks — list server-owned bookmarks/folders. Active tab opens the
// editor; Deleted tab offers restore. New bookmark via the header button.
...
import { ChevronRight, Bookmark, Folder } from "lucide-react-native";
...
export default function AdminBookmarks() {
  ...
  async function restore(item) {
    ...
    setBusyId(item.id);
    try {
      await client.admin.restoreBookmark({ bookmarkId: item.id });
      setItems((arr) => arr.filter((b) => b.id !== item.id));
    } catch (e) {
  ...
  return (
    <SafeAreaView className="flex-1 bg-base-100" edges={["left", "right"]}>
      <AppHeader
        back
        title="Bookmarks"
        right={<HeaderButton label="New" onPress={() => router.push("/admin/bookmark/new")} />}
      />
        ...
        {loading ? (
          ...
          <View className="px-6 py-20 items-center">
            <Text className="font-ui text-base text-base-content/70 text-center mb-1">
              {isDeleted ? "No deleted bookmarks." : "No bookmarks yet."}
            </Text>
            {!isDeleted ? (
              <Text className="font-ui text-sm text-base-content/45 text-center leading-6">
                Tap New to create your first bookmark.
              </Text>
            ) : null}
          </View>
        ) : (
            ...
            return (
              <Pressable
                ...
                onPress={() =>
                  isDeleted
                    ? null
                    : router.push(`/admin/bookmark/${encodeURIComponent(item.id)}`)
                }
                android_ripple={{ color: "rgba(0,0,0,0.05)" }}
              ...
              >
                <View className="mr-3">
                  {isFolder ? (
                    ...
                    <Bookmark size={18} color={ink(0.6)} strokeWidth={1.75} />
                  )}
                </View>
