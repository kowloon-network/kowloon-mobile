// Edit a server-owned group (or delete it). 403s if the group wasn't
...
export default function EditAdminGroup() {
  ...
  const load = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const res = await client.admin.getGroup({ groupId: String(id) });
      setGroup(res?.group || res?.item || res || null);
    } catch (e) {
  ...
  function handleDelete() {
    Alert.alert("Delete group?", "It can be restored later from the Deleted tab.", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            await client.admin.deleteGroup({ groupId: String(id) });
            router.back();
          } catch (e) {
  ...
  return (
    <SafeAreaView className="flex-1 bg-base-100" edges={["left", "right"]}>
      <AppHeader
        back
        title="Edit Group"
        right={group ? <HeaderButton label="Delete" onPress={handleDelete} /> : null}
      />
