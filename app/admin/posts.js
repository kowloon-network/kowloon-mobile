// Admin posts — list public/server posts. Active tab opens the editor
// (403s server-side if not server-owned); Deleted tab offers restore.
// New post via the header button.
...
export default function AdminPosts() {
  ...
  async function restore(post) {
    ...
    setBusyId(post.id);
    try {
      await client.admin.restorePost({ postId: post.id });
      setItems((arr) => arr.filter((p) => p.id !== post.id));
    } catch (e) {
  ...
  return (
    <SafeAreaView className="flex-1 bg-base-100" edges={["left", "right"]}>
      <AppHeader
        back
        title="Posts"
        right={<HeaderButton label="New" onPress={() => router.push("/admin/post/new")} />}
      />
        ...
        {loading ? (
          ...
          <View className="px-6 py-20 items-center">
            <Text className="font-ui text-base text-base-content/70 text-center mb-1">
              {isDeleted ? "No deleted posts." : "No posts yet."}
            </Text>
            {!isDeleted ? (
          ...
          items.map((post) => (
            <Pressable
              ...
              onPress={() =>
                isDeleted
                  ? null
                  : router.push(`/admin/post/${encodeURIComponent(post.id)}`)
              }
              android_ripple={{ color: "rgba(0,0,0,0.05)" }}
            ...
            >
              <View
                ...
                style={{ backgroundColor: TYPE_COLORS[post.type] || ink(0.3) }}
              />
              <View className="flex-1 min-w-0">
                ...
                <Text className="font-ui text-xs text-base-content/50" numberOfLines={1}>
                  {post.type} · {post.actorId}
                </Text>
              </View>
              {isDeleted ? (
                <Pressable
                  onPress={() => restore(post)}
