// Admin groups — list server-owned + user-owned groups. Active tab opens
// the editor (403s server-side if not server-owned); Deleted tab offers
// restore. New group via the header button.
...
export default function AdminGroups() {
  ...
  async function restore(group) {
    ...
    setBusyId(group.id);
    try {
      await client.admin.restoreGroup({ groupId: group.id });
      setItems((arr) => arr.filter((g) => g.id !== group.id));
    } catch (e) {
  ...
  return (
    <SafeAreaView className="flex-1 bg-base-100" edges={["left", "right"]}>
      <AppHeader
        back
        title="Groups"
        right={<HeaderButton label="New" onPress={() => router.push("/admin/group/new")} />}
      />
        ...
        {loading ? (
          ...
          <View className="px-6 py-20 items-center">
            <Text className="font-ui text-base text-base-content/70 text-center mb-1">
              {isDeleted ? "No deleted groups." : "No groups yet."}
            </Text>
            {!isDeleted ? (
          ...
          items.map((group) => (
            <Pressable
              ...
              onPress={() =>
                isDeleted
                  ? null
                  : router.push(`/admin/group/${encodeURIComponent(group.id)}`)
              }
              android_ripple={{ color: "rgba(0,0,0,0.05)" }}
            ...
            >
              ...
              <View className="flex-1 min-w-0 ml-3">
                ...
                <Text className="font-ui text-xs text-base-content/50" numberOfLines={1}>
                  {group.actorId} · {group.memberCount ?? 0} member
